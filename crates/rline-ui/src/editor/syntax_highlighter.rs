...
//!
//! Creates GTK `TextTag` objects colored from the active sourceview5 style scheme,
//! then applies them to the buffer based on [`HighlightSpan`] output from the
//! [`HighlightEngine`].
...
/// objects derived from the active style scheme. Connects to buffer change
/// signals for incremental re-highlighting.
...
impl SyntaxHighlighter {
    ...
    /// creates TextTags from the active style scheme, and connects buffer change
    /// signals for incremental re-highlighting.
    ///
    /// # Errors
    ///
    ...
    pub fn new(
        ...
        // Build TextTags from the active style scheme
    ...
    /// Rebuild TextTags from the current style scheme and re-highlight.
    ///
    ...
    /// Build TextTags for each highlight group from the buffer's style scheme.
    fn build_tags(buffer: &sourceview5::Buffer) -> HashMap<usize, gtk4::TextTag> {
        ...
        for (index, &_name) in HIGHLIGHT_NAMES.iter().enumerate() {
            let Some(style_id) = scope_map::highlight_to_style_id(index) else {
                continue;
            };

            // Look up the style in the active scheme
            let style = scheme.as_ref().and_then(|s| s.style(style_id));
            ...
            if let Some(ref style) = style {
                ...
                if style.is_bold_set() && style.is_bold() {
                    tag.set_weight(700); // pango::Weight::Bold = 700
                }
                // Apply italic
    ...
    fn apply_spans(&self, spans: &[rline_syntax::HighlightSpan], source: &[u8]) {
        for span in spans {
            ...
            // Convert byte offsets to TextIter positions.
            // We use line/byte-within-line to avoid UTF-8 char offset mismatches.
    ...
    fn byte_offset_to_iter(&self, source: &[u8], byte_offset: usize) -> gtk4::TextIter {
        ...
        // Count the line and byte-within-line
        let mut line: i32 = 0;
        ...
        // iter_at_line_index takes a line number and a byte offset within that line
        self.buffer
    ...
    fn connect_buffer_signals(&mut self) {
        ...
        // We use connect_after to run after the text has been inserted/deleted.
        // Both signals schedule the same idle re-highlight.
            ...
            *is_scheduled = true;
            ...
            glib::idle_add_local_once(move || {
                *scheduled.borrow_mut() = false;
                ...
                // For simplicity, do a full re-highlight on each edit.
                // Tree-sitter parsing is <1ms for typical files, so this is fine.
                // The incremental API is available in HighlightEngine for future
                // optimization if profiling shows this is a bottleneck.
